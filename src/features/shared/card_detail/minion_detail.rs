...
use crate::{
    ...
    features::{cards::MinionCard, shared::text_with_background::TextWithBackgroundBuilder},
};

pub fn spawn_minion_detail(
    ...
    children_builder
        ...
        .with_children(|container| {
            spawn_image(container, asset_server.load(minion.card_image_path));
            spawn_detail(container, asset_server, minion.clone());
            spawn_footer(container, minion.id);
        });
}
    ...
    container
        ...
        .with_children(|detail_container| {
            let mut text_builder = TextWithBackgroundBuilder {
                text: format!("{} {}", if minion.unique { "+" } else { "" }, minion.name),
                style: Style {
            ...
            text_builder.text = minion.description.to_string();
...
fn spawn_footer(container: &mut ChildBuilder, minion_id: &str) {
    TextWithBackgroundBuilder {
        text: minion_id.to_string(),
        style: Style {
            position_type: PositionType::Absolute,
            right: Val::Px(30.),
            bottom: Val::Px(10.),
