...
use crate::{
    ...
    features::{cards::AlterEgoCard, shared::text_with_background::TextWithBackgroundBuilder},
};

pub fn spawn_alter_ego_detail(
    children_builder: &mut ChildBuilder,
    asset_server: Res<AssetServer>,
    alter_ego: AlterEgoCard<'static>,
) {
    children_builder
        .spawn({
            NodeBundle {
        ...
        .with_children(|container| {
            spawn_image(container, asset_server.load(alter_ego.card_image_path));
            spawn_detail(container, asset_server, alter_ego.clone());
            spawn_footer(container, alter_ego.id, alter_ego.flip_target_id);
        });
}
...
fn spawn_detail(
    ...
    container
        ...
        .with_children(|detail_container| {
            ...
            text_builder.text = format!("HP: {}", alter_ego.initial_hit_points);
            text_builder.style.grid_column = GridPlacement::span(1);
            text_builder.spawn(detail_container);

            text_builder.text = format!("REC: {}", alter_ego.rec);
            text_builder.spawn(detail_container);

            text_builder.text = format!("Hand Size: {}", alter_ego.hand_size);
            text_builder.style.grid_column = GridPlacement::span(2);
            text_builder.spawn(detail_container);

            let traits_text: Vec<String> = alter_ego
                .traits
                .iter()
            ...
            let keyword_text: Vec<String> = alter_ego
                .keywords
                .iter()
                .map(|item| item.to_string())
                .collect();
            ...
            text_builder.text = alter_ego.description.to_string();
                ...
                .with_children(|desciprtion_container| {
                    desciprtion_container
                        ...
                        .with_children(|icons_container| {
                            for icon in alter_ego.card_icons {
